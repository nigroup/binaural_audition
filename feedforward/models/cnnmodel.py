...
def model(hyperparams,x):
    ...
    def conv3d_layer_with_pooling(
        ...
        # (bs*channel)* cf* time*mf
        maxpool = tf.nn.max_pool(activation_reshaped, ksize=pool_window_size.tolist(), strides=pool_strides.tolist(),
                                 padding="SAME")

        # bs*cf*mf*time*channel - back to shape bevore reshaping
        maxpool_reshaped = tf.reshape(maxpool,
                                      [-1, maxpool.shape[1], maxpool.shape[3], maxpool.shape[2], activation.shape[4]])
    ...
    def buildRatemapConvolution(x):
        ...
        conv1 = conv2d_layer_with_pooling(x_ratemap, hyperparams["ratemap_ksize"], np.array([1]), hyperparams["feature_maps_layer"],
                                          np.array([30, 48]), hyperparams["sequence_ratemap_pool_window_size"][0],
                                          hyperparams["sequence_ratemap_pool_strides"][0])


        conv2 = conv2d_layer_with_pooling(conv1 ,hyperparams["ratemap_ksize"], np.array([conv1.shape[3].value]), hyperparams["feature_maps_layer"],
                                          np.array([30, 48]), hyperparams["sequence_ratemap_pool_window_size"][1],
                                          hyperparams["sequence_ratemap_pool_strides"][1])

        conv3 = conv2d_layer_with_pooling(conv2, hyperparams["ratemap_ksize"], np.array([conv2.shape[3].value]), hyperparams["feature_maps_layer"],
        ...
        if hyperparams["nr_conv_layers_ratemap"] == 4:
            conv4 = conv2d_layer_with_pooling(conv3, hyperparams["ratemap_ksize"], np.array([conv3.shape[3].value]),
                                              ...
                                              hyperparams["sequence_ratemap_pool_window_size"][3], hyperparams["sequence_ratemap_pool_strides"][3])
            return conv4
    ...
    def buildAMSConvolution(x):
        ...
        conv1 = conv3d_layer_with_pooling(x_split_ams, hyperparams["ams_ksize"], np.array([1]), hyperparams["feature_maps_layer"], None,
                                          hyperparams["sequence_ams_pool_window_size"][0], hyperparams["sequence_ams_pool_strides"][0])

        conv2 = conv3d_layer_with_pooling(conv1, hyperparams["ams_ksize"], np.array([conv1.shape[4].value]), hyperparams["feature_maps_layer"], None,
                                          hyperparams["sequence_ams_pool_window_size"][1], hyperparams["sequence_ams_pool_strides"][1])
        conv3 = conv3d_layer_with_pooling(conv2, hyperparams["ams_ksize"], np.array([conv2.shape[4].value]), hyperparams["feature_maps_layer"], None,
                                          hyperparams["sequence_ams_pool_window_size"][2], hyperparams["sequence_ams_pool_strides"][2])

        if hyperparams["nr_conv_layers_ratemap"] == 4:
            conv4 = conv3d_layer_with_pooling(conv3, hyperparams["ams_ksize"], np.array([conv3.shape[4].value]), hyperparams["feature_maps_layer"],
                                              None,  hyperparams["sequence_ams_pool_window_size"][3], hyperparams["sequence_ams_pool_strides"][3])
            return conv4
    ...
    flatLayerRatemap = tf.reshape(lastconvRatemap, [-1,
                                                    lastconvRatemap.shape[1].value * lastconvRatemap.shape[2].value *
                                                    lastconvRatemap.shape[3].value])
    flatLayerAMS = tf.reshape(lastconvAMS, [-1,
                                            lastconvAMS.shape[1].value * lastconvAMS.shape[2].value * lastconvAMS.shape[
