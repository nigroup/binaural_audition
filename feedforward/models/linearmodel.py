...
def run_model(nr_neurons_hidden_layer, epochs):
    ...
    w1 = tf.Variable(tf.random_normal(shape=(framelength*n_features,nr_neurons_hidden_layer), stddev=1), name='w1') #sinvoll: 1 #gradient ungleich 0
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(epochs):
            ...
            for i in range(total_batch):
                ...
                summary, cost, cp, yprint, y_print, acc = sess.run([merged, cross_entropy, correct_prediction,y,y_,accuracy], feed_dict={x: batch_x, y:batch_y})
            ...
            if epoch%10==0:
                ...
                summary, yy,yy_,cp,acc = sess.run([merged, y,y_,correct_prediction,accuracy], feed_dict={x: val_x, y: val_y})
                print("test writer")
