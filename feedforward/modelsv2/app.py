...
model_name = 'CNN_v1'
save_path = os.path.join('/Users/alessandroschneider/Desktop/TuBerlin/bachelor/log', model_name)
    ...
    for i_val_fold, val_fold in enumerate(h.ALL_FOLDS):
        ...
        with tf.Session(graph=g) as sess:
            ...
            train_loader = ldataloader.LineDataLoader('train', h.LABEL_MODE, TRAIN_FOLDS, h.TRAIN_SCENES,
                                                      ldl_timesteps=hyperparams["ldl_timesteps"], ldl_batchsize=h.LDL_BATCHSIZE,
                                                      ldl_overlap=h.LDL_OVERLAP,
                                                      ldl_lines_per_batch=h.LDL_LINES_PER_BATCH,
                                                      epochs=h.EPOCHS, features=h.NFEATURES, classes=h.NCLASSES, ldl_buffer=h.LDL_BUFFER)
            ...
            # training phase
            train_phase = tr_utils.Phase('train', graphModel, sess, train_loader, *args)

            # validation phase
            val_phase = tr_utils.Phase('val', graphModel, sess, val_loader, *args)
            ...
            for e in range(h.epochs_finished[i_val_fold], h.EPOCHS):
                ...
                heiner_utils.pickle_metrics(metrics, model_save_dir)
            ...
            heiner_utils.pickle_metrics(metrics, model_dir)
