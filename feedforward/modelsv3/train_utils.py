...
class Phase:
    ...
    def __init__(self, train_or_val, graphModel, session, dloader, hyperparams,  loss_weights, OUTPUT_THRESHOLD, MASK_VAL, EPOCHS, val_fold_str, metric='BAC',
                 ret=('final', 'per_class')):
    ...
    def run(self):
        ...
        for iteration in range(1, 3):
            ...
            it_str = '{}_iteration: {} / {}'.format(self.prefix, iteration, 3)#self.dloader_len[self.e])

            b_x, b_y = next(self.gen)

            if self.train:
                b_x = b_x.reshape((-1, settings.timesteps, settings.n_features))

                b_y = b_y[:, :, -1, :, :]
                b_y_no_sceneIds = b_y[:, :, :, 0].reshape((-1, settings.n_labels))
                #train
                _,loss =  self.session.run([self.graphModel.optimiser,self.graphModel.cross_entropy], feed_dict={self.graphModel.x: b_x, self.graphModel.y: b_y_no_sceneIds, self.graphModel.cross_entropy_class_weights : self.loss_weights })
                #predict on batch
                out =  self.session.run([self.graphModel.thresholded], feed_dict={self.graphModel.x : b_x, self.graphModel.y:b_y_no_sceneIds})
            else:

                #make divisible
                b_x = b_x[:, 0:b_x.shape[1]-b_x.shape[1]%settings.ldl_timesteps, :]
                b_y = b_y[:, 0:b_y.shape[1]-b_y.shape[1]%settings.ldl_timesteps, :, :]

                #y:no sceneIds
                b_y = b_y[:, :, :, 0] #(40, 19698, 13)


                #format von cnn model
                b_x = b_x.reshape((-1, settings.ldl_timesteps, settings.n_features))
                b_y =b_y.reshape((-1,settings.n_labels))


                #y:take last label only
                b_y = b_y[::settings.timesteps,:]



                #loss onn batch, no train
                loss =  self.session.run([self.graphModel.optimiser,self.graphModel.cross_entropy], feed_dict={self.graphModel.x: b_x, self.graphModel.y: b_y}) #self.graphModel.cross_entropy_class_weights : data.cross_entropy_class_weights})
                #predict on batch
                out =  self.session.run([self.graphModel.thresholded], feed_dict={self.graphModel.x: b_x, self.graphModel.y:b_y })
