...
def compare_blockbased(gpu):
    ...
    model_2 = model_definition(BATCH_SIZE)

    adam = Adam(lr=0.001, clipnorm=1.)
    model_2.compile(optimizer=adam, loss=my_loss, metrics=None)
    ...
    val_loader_2 = tr_utils.create_val_dataloader('blockbased', -1, BATCH_SIZE, 1000, 1, 160, 13,
                                                [3], True, 100, use_multithreading=False, input_standardization=False)

    args = [0.5, -1, 1, 'val fold 3', False, 0.0, 'BAC']
    # validation phase
    val_phase_2 = tr_utils.Phase('val', model_2, val_loader_2, 100, *args)

    # TODO change it back
    has_nan_val_2, sid_dict_val_2 = val_phase_2.run()
    ...
    save_dir = '/home/spiess/twoears_proj/models/heiner/test_blockbased_validation/'
    save_dir_val = os.path.join(save_dir, 'val_2')
    os.makedirs(save_dir_val, exist_ok=True)
    utils.pickle_metrics(metrics_2, save_dir_val)
