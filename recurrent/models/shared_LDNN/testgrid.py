...
from model_loader_grid import MultiRNN
...
class HyperParameters:
    def __init__(self, VAL_FOLD, FOLD_NAME):
        ...
        # Parameters for LDNNlayer
        self.NUM_HIDDEN = 128
        self.NUM_LSTM = 1
        self.NUM_NEURON = 128
        self.NUM_MLP = 1
        # Parameters for grid layer
        self.NUM_GRID = 50
        self.STRIDE = 40
        self.FILTESIZE = 80
        self.SHARING = False
    ...
    def validation(self, batch_size):
        with tf.name_scope('LDNN'):
            ...
            logits, update_op, reset_op = MultiRNN(X, batch_size, seq, self.NUM_CLASSES,
                                         ...
                                         self.NUM_MLP, self.NUM_NEURON,self.NUM_GRID,self.STRIDE,self.FILTESIZE, training=False,SHARING=self.SHARING)
    ...
    def train(self, batch_size):
        with tf.name_scope('LDNN'):
            ...
            logits, update_op, _ = MultiRNN(X, batch_size, seq, self.NUM_CLASSES,
                                         self.NUM_LSTM, self.NUM_HIDDEN, self.OUTPUT_KEEP_PROB,
                                         self.NUM_MLP, self.NUM_NEURON,self.NUM_GRID,self.STRIDE,self.FILTESIZE, training=True, SHARING=self.SHARING)
