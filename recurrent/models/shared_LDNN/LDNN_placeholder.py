...
class HyperParameters:
    ...
    def next_batch(self,index, mode):
        files = self.SET[mode][index:index+self.BATCH_SIZE]
        fx, fy, fl = np.array([]).reshape(0, 160), np.array([]).reshape(0, 13), np.array([]).reshape(0)
        for file in files:
            x,y,l = self._read_py_function(file)
            fx = np.concatenate((fx,x))
            fy = np.concatenate((fy, y))
            fl = np.concatenate((fl, l))
        fx = fx.reshape((-1,self.TIMELENGTH,160))
        fy = fy.reshape((-1, self.TIMELENGTH, 13))
        fl = fl.reshape((-1, 1))
        return fx,fy,fl
    ...
    def main(self):
        # tensor holder
        self.feature = tf.placeholder(tf.float32, shape=[None, None, 160])
        self.label = tf.placeholder(tf.int32, shape=[None, None, 13])
        self.lens = tf.placeholder(tf.int32, shape=[None, None])

        X, Y, seq = self.feature,self.label,self.lens
        ...
        logits, update_op = self.MultiRNN(X, weights)
        ...
        with tf.Session() as sess:
            ...
            for num in range(n_batches):
                x, y , l = self.next_batch(num,'train')
                feed = {self.feature: x,
                        self.label: y,
                        self.lens: l}
                ...
                if (num % batch_per_epoch == 0 and num != 0):
                    ...
                    for tnum in range(v_batches_per_epoch):
                        x, y, l = self.next_batch(tnum, 'test')
                        feed = {self.feature: x,
                                self.label: y,
                                self.lens: l}
                        se, sp, tempf1 = sess.run([sensitivity, specificity, f1], feed_dict=feed)
                        sen = sen + se
                        spe = spe + sp
                        f = tempf1 + f
